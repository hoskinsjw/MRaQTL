...
#'
#' @return Writes the density plot for all regulators' matched activities and expression values Pearson correlations to a png image file.
#' @export
#'
#' @examples
#' \dontrun{
#' corDensPlot(reg_expression,reg_activity,"Regulators")
#' # Generates the file "Regulators_activity_expression_correlation.png" in your working directory.
#' }
corDensPlot <- function(expression,activity,file_prefix){
  ...
  expActCor=list()
  startTime=Sys.time()
  expActCor=foreach(i=1:dim(expression)[1]) %dopar%
    cor(t(expression)[,i],t(activity)[,i])
  expActCor=unlist(expActCor) # Very rapidly convert a list to data.frame
  stopCluster(cl)
  Sys.time()-startTime
  names(expActCor)=rownames(expression)

  png(paste(file_prefix,"_activity_expression_correlation.png",sep=""),width = 800,height = 600)

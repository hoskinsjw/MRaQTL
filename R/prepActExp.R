...
#'
#' @param gene_exp string for the file name of the expression data file (should be appropriately normalized and pre-filtered for expression).
#' @param covariables string for the file name of the covariables data file.
#' @param network string for the file name of an ARACNe network file. IMPORTANT: this file cannot have a header.
#' @param prefix string indicating a prefix to be added to all output file names.
#'
#' @return Writes six output files:
#' \enumerate{
#'   \item  A log file recording the standard outputs generated by the function:
#'   \item  A tab-delimited file for the interactome/regulon generated by the aracne2regulon() function from the input ARACNe network and expression data.
#'   \item  A png image file of a density plot for all regulators' matched activities and expression values Pearson correlations.
#'   \item  A tab-delimited file containing the quantile normalized and inverse normal transformed expression values for all input genes.
#'   \item  A tab-delimited file containing the quantile normalized and inverse normal transformed expression values for only expression regulator genes for which activities were successfully inferred.
#'   \item  A tab-delimited file containing the activities inferred for expression regulator genes by VIPER.
#' }
#' @export
#'
#' @examples
#' \dontrun{
#' prepActExp("GEUVADIS_filtered_samples_expressed_genes_logRPKMs_with_symbols.txt",
...
prepActExp <- function(gene_exp,covariables,network,prefix){
  ...
  genes=read.table(gene_exp,sep = "\t",header = T, row.names = 1)
  genes_set=ExpressionSet(assayData=as.matrix(genes))
  covars=as.data.frame(t(read.table(covariables,sep="\t",header = T,row.names = 1)))
  ...
  regulon=aracne2regulon(network,genes_set,format="3col") # NOTE: The aracne network file cannot have a header!!!

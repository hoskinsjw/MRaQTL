...
#'
#' @param rfcv_workspace string indicating the path and file name of the .RData
#' workspace image file from the random forest cross-validation analysis
#' generated by the rfCrossVal function.
#' @param exp_file string indicating the path and file name of the normalized
...
#'   expression data filtered to include on the representative phenotypic MRs.
#'   \item  A tab-delimited text file containing the activity data (which was
#'   already present in the input workspace file) filtered to include on the
...
finalRF <- function(rfcv_workspace,exp_file,phenotype,prefix,mr_count=100){
  ...
  # Read in workspace and expression data
  print("Reading in workspace from random forest cross-validation analysis and expression data")
  load(rfcv_workspace)
  ...
  # Write the MRs expression and activity data to file
  grabMRs(exp_file,vip,mrs,prefix)

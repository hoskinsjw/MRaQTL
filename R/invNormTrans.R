...
#'
#' @param genes matrix or data.frame containing gene metrics with genes in rows and samples in columns. Gene and sample identifiers should be in rownames and colnames, respectively.
#'
#' @return Returns a data.frame in the same format as the input gene data, but with inverse normal transformed values.
#' @export
#'
#' @examples
#' \dontrun{
...
invNormTrans <- function(genes){
  ...
  int_exp=list()
  int_exp=foreach(i=1:dim(genes)[1],.packages = 'RNOmni') %dopar%
    RankNorm(as.numeric(genes[i,]))
